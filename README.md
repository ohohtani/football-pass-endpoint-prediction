...
- 원본 대회 데이터는 저작권 / 대회 규정 문제로 업로드하지 않는 것을 권장합니다.
- `data/`, `outputs/`, `submission.csv`, 모델 파일은 `.gitignore`로 제외하는 구성이 적절합니다.
- 깃허브에는 **코드 구조와 실행 흐름이 잘 보이도록** 소스만 올리는 편이 가장 깔끔합니다.
...
git init
git add .
git commit -m "Refactor football pass endpoint prediction pipeline"
```
...
원래는 전처리, 피처 생성, 모델 학습, 추론, 저장까지 하나의 파일에 모두 들어 있었기 때문에,
코드를 다시 볼 때도 흐름을 찾기 어렵고 GitHub에서도 구조가 잘 드러나지 않는 문제가 있었습니다.
...
- 모델 로직 분리
- 학습 / 추론 분리
를 통해, **실험 코드는 유지하면서도 읽기 쉬운 프로젝트 구조**가 되도록 정리했습니다.
